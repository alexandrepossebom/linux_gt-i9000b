...
#else
			gpio_direction_output(_3_GPIO_TOUCH_EN, 0);
 /*lhs_2010.09.14 for bluetooth bt off issue
Cause : We called gpio function to set non-use pin _3_GPIO_TOUCH_CE
          *caution : we must set non-use pin to null.
Resolution : we blocked gpio funtion to set non-use pin */
#if !defined(CONFIG_ARIES_NTT)&&!defined(CONFIG_LATIN_ARIES_TV) 
			gpio_direction_output(_3_GPIO_TOUCH_CE, 0);
#endif
			gpio_direction_output(_3_TOUCH_SDA_28V, 0);
			gpio_direction_output(_3_TOUCH_SCL_28V, 0);
...
#else
	gpio_direction_output(_3_GPIO_TOUCH_EN, 0);
#if !defined(CONFIG_ARIES_NTT)&&!defined(CONFIG_LATIN_ARIES_TV) //lhs_2010.09.14 for bluetooth bt off issue
	gpio_direction_output(_3_GPIO_TOUCH_CE, 0);
#endif
	gpio_direction_output(_3_TOUCH_SDA_28V, 0);
	gpio_direction_output(_3_TOUCH_SCL_28V, 0);
