...
// latin_cam : VT Cam Anti-banding
static int s5ka3dfx_set_60hz_anti_banding(struct v4l2_subdev *sd)
{
	unsigned char s5ka3dfx_anti_banding_60Hz[][2] =
	{
	...
	int err= -EINVAL, i, s5ka3dfx_anti_banding_regs;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct s5ka3dfx_state *state = to_state(sd);	
	
	s5ka3dfx_anti_banding_regs = (sizeof(s5ka3dfx_anti_banding_60Hz) / sizeof(s5ka3dfx_anti_banding_60Hz[0]));
	
	for (i = 0; i < s5ka3dfx_anti_banding_regs; i++) 
	{
		err = s5ka3dfx_i2c_write(sd, s5ka3dfx_anti_banding_60Hz[i], sizeof(s5ka3dfx_anti_banding_60Hz[i]));
		if (err < 0)
		{
...
static int s5ka3dfx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	{
		err = s5ka3dfx_set_60hz_anti_banding(sd);
		if (err < 0) 
		{
