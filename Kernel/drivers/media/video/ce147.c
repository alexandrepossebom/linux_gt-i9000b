...
struct ce147_state {
	...
	int exif_ctrl;//ykh
	int thumb_null;//ykh
	int exif_modelname; //latin_cam : support exif model name, hmin84.park 100817	
};
...
static int ce147_set_anti_banding(struct v4l2_subdev *sd)
{
	...
	dev_err(&client->dev, "Anti-Banding = %d\n", state->anti_banding);	//latin_cam : support anti banding, hmin84.park 100817
	ce147_msg(&client->dev, "%s: done\n", __func__);
...
static int ce147_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if(state->jpeg.enable){
		...
		//latin_cam : jyc_support exif model name
		/*
		 * 4-1. Set EXIF information(Model Name)
		 */
		err = ce147_set_exif_model_name(sd);
		if(err < 0){
			dev_err(&client->dev, "%s: failed: i2c_write for exif - Model Name\n", __func__);
			return -EIO;
		}
